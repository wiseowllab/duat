...
export class GameScene extends Phaser.Scene {
  ...
  showUnlockEvents(unlockEvents) {
    ...
    this.showGodUnlockFeedback(latestUnlock);
    this.showGodAwakenOverlay(latestUnlock);
  ...
  showGodAwakenOverlay(unlockEvent) {
    if (!unlockEvent?.god || this.gameState !== GAME_STATES.PLAYING) {
      return;
    }

    const boardCenterX = BOARD_ORIGIN_X + (BOARD_COLUMNS * CELL_SIZE) / 2;
    const boardCenterY = BOARD_ORIGIN_Y + (BOARD_ROWS * CELL_SIZE) / 2;
    const tier = Phaser.Math.Clamp(unlockEvent.god.tier ?? 1, 1, 4);
    const panelWidth = Math.min(328, BOARD_COLUMNS * CELL_SIZE + 28);
    const panelHeight = Math.min(212, BOARD_ROWS * CELL_SIZE - 18);
    const borderColor = tier >= 3 ? 0x8ed9af : 0xd4af37;
    const accentColor = tier >= 2 ? 0x62c2ff : 0xf4d77a;
    const overlayDuration = 1200 + tier * 140;
    const stock = this.bombSystem.getStock();
    let latestBomb = null;
    for (let index = stock.length - 1; index >= 0; index -= 1) {
      if (stock[index]?.godId === unlockEvent.god.id) {
        latestBomb = stock[index];
        break;
      }
    }
    const bombLabel = latestBomb ? (BOMB_LABELS_JA[latestBomb.type] ?? latestBomb.name) : 'なし';
    const isRa = unlockEvent.god.id === 'ra';
    const isAmunRa = unlockEvent.god.id === 'amun_ra';
    const awakenedTitle = isRa
      ? 'ラー、目覚める'
      : isAmunRa
        ? 'アメン＝ラー、目覚める'
        : '神、目覚める';

    this.godAwakenOverlay?.destroy(true);
    this.godAwakenHideTimer?.remove(false);
    this.godAwakenTween?.stop();
    this.godAwakenFlashTween?.stop();

    const container = this.add.container(boardCenterX, boardCenterY).setDepth(28).setAlpha(0);
    const bgGlow = this.add.ellipse(0, 0, panelWidth + 36 + tier * 16, panelHeight + 26 + tier * 10, 0xd4af37, 0.14 + tier * 0.03);
    const panel = this.add.rectangle(0, 0, panelWidth, panelHeight, 0x080704, 0.86)
      .setStrokeStyle(2, borderColor, 0.95);
    const inner = this.add.rectangle(0, 0, panelWidth - 16, panelHeight - 16, 0x1a120a, 0.72)
      .setStrokeStyle(1, accentColor, 0.44);

    const titleText = this.add.text(0, -64, awakenedTitle, {
      fontFamily: 'Georgia, serif',
      fontSize: tier >= 4 ? '30px' : '26px',
      color: '#f4d77a',
      fontStyle: 'bold',
      stroke: '#120b05',
      strokeThickness: 4,
    ...
    const detailLines = [
      `神名: ${unlockEvent.god.name}`,
      `Tier ${unlockEvent.god.tier}: ${unlockEvent.god.tierName}`,
      `授与ボム: ${bombLabel}`,
    ];
    if (unlockEvent.isComplete) {
      detailLines.push('DUAT COMPLETE');
    }
    const detailText = this.add.text(0, 14, detailLines.join('\n'), {
      fontFamily: 'Arial, sans-serif',
      fontSize: '18px',
      color: '#eadfca',
      align: 'center',
    ...
    container.add([bgGlow, panel, inner, titleText, detailText]);
    container.setScale(0.92);
    this.godAwakenOverlay = container;

    if (tier >= 4) {
      const boardFlash = this.add.rectangle(boardCenterX, boardCenterY, BOARD_COLUMNS * CELL_SIZE + 16, BOARD_ROWS * CELL_SIZE + 16, 0xf4d77a, 0)
        .setDepth(27);
    ...
    this.godAwakenTween = this.tweens.add({
      targets: container,
      alpha: { from: 0, to: 1 },
      scaleX: { from: 0.92, to: 1.02 + tier * 0.01 },
      scaleY: { from: 0.92, to: 1.02 + tier * 0.01 },
      duration: 260,
      ease: 'Back.easeOut',
      yoyo: true,
      hold: overlayDuration,
      onComplete: () => {
        this.tweens.add({
          targets: container,
          alpha: { from: container.alpha, to: 0 },
          scaleX: { from: container.scaleX, to: 0.96 },
          scaleY: { from: container.scaleY, to: 0.96 },
          duration: 260,
          ease: 'Sine.easeIn',
          onComplete: () => {
